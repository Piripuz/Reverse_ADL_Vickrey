...
def _pdf_sched(mu, sigma):
    def inner_pdf(x):
        return jtruncnorm.pdf(
            x, (0.01 - mu) / sigma, 10000, loc=mu, scale=sigma
        )

    return inner_pdf


def _cdf_sched(mu, sigma):
    def inner_cdf(x):
...
def _ot_lik(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t):
    cdf_b = _cdf_sched(mu_b, sigma)
    cdf_g = _cdf_sched(mu_g, sigma)
    ...
    return likelihood_kink


def _early_lik(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t):
    pdf_g = _pdf_sched(mu_g, sigma)
    pdf_b = _pdf_sched(mu_b, sigma)
...
def _late_lik(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t):
    pdf_g = _pdf_sched(mu_g, sigma)
    pdf_b = _pdf_sched(mu_b, sigma)
...
def likelihood(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t):
    ...
    """
    lik_early = _early_lik(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t)
    lik_late = _late_lik(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t)
    likelihood_kink = _ot_lik(
